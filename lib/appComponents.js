...
AppComponents.prototype.Layout = function (settings) {
  ...
  var sizeCell = function (cellHtml) {
    ...
    var toolbarHt = cellHtml.find(".toolbar")
      ? cellHtml.find(".toolbar").outerHeight()
      : 0;
...
AppComponents.prototype.Grid = function (settings) {
  ...
  var createHtml = function () {
    ...
    var d = $("<div>");
    d1 = d.clone();
    d1.addClass("div-head");
    d2 = d.clone();
    d2.addClass("div-body");
    ...
    if (html.is(":visible") && !_isEmpty()) {
      ...
      if (!_isEmpty()) {
        ...
        $.each(tds, function (i, td) {
          var wd = settings.columns[i].width
            ? settings.columns[i].width
            : $(td).width();
          wd +=
            i == tds.length - 1 && tableHeader.width() > tableBody.width()
  ...
  var loadData = function (dataArray, pageNumber) {
    ...
    if (!records) {
      inner.append(
        getRow([{ dataKey: "description" }], {
          id: "no-records",
          description: "0 Records",
        })
      );
    }
    setColumnWidths();
  ...
  var setPaging = function (length, pageNumber) {
    ...
    d2.scrollTop();
    tableFooter
      .find("[data-pagenumber='" + pageNumber + "']")
      .addClass("active");
  ...
  var getRow = function (columns, dataRow) {
    ...
    $.each(columns, function (i, column) {
      ...
      if (!column.dataKey && column.html) {
        ...
        var val = column.getValue
          ? column.getValue(dataRow[column.dataKey])
          : dataRow[column.dataKey];
        _td.html(val);
      }
      if (column.onClick) {
        _td.click(function () {
  ...
  var _isEmpty = function () {
    ...
    return (
      !trs.length ||
      (trs.length == 1 && $(trs[0]).attr("data-id") == "no-records")
    );
  };
  ...
  var bindKeyboardNavigation = function () {
    ...
    t.on("keydown", function (e) {
      if (
        e.which == constants.keycode.up ||
        e.which == constants.keycode.down
      ) {
        move(e.which);
...
AppComponents.prototype.Form = function (settings) {
  /**
    ...
    **/
  var self = this;
  var settings = { ...settings, labelWidth: 100 };
  var html;
  var loadedData;
  ...
  var __construct = function () {
    ...
    self.focus = function () {
      html
        .find("input:not([readonly]), select:not([readonly])")
        .first()
        .focus();
    };
  ...
  var _runCustomValues = function () {
    if (hasCustomValue) {
      $.each(settings.fields, function (i, field) {
        if (field.customValue) {
          html
            .find("[name='" + field.dataKey + "']")
            .val(field.customValue(self));
        }
      });
    }
  };
...
AppComponents.prototype.Popup = function (settings) {
  ...
  var settings = $.extend({ width: 400 }, settings);
  var html;
  var body;
  var buttons;
  var overlay;
  ...
  var style = {
    overlay:
      "position: fixed; top: 0px; height: 100%; width: 100%; z-index: 9999;",
    html: "position: fixed; width: " + settings.width + "px; z-index: 9999;",
    body: "",
    buttons: "text-align: right; height: 33px;",
    button:
      "border: 0; height: 33px; line-height: 33px; padding: 0 10px; background: none; cursor: pointer; margin: 0 15px 0 0; ",
    buttonHover: "",
  };
...
AppComponents.prototype.EditorGrid = function (
  settings,
  Toolbar,
  Grid,
  Popup,
  ...
  var beforeSaveCallback = settings.beforeSaveCallback
    ? settings.beforeSaveCallback
    : function (data, allowCb) {
        allowCb();
      };
  var saveCallback = settings.saveCallback
