    ...
    var domFileStorage = new DOMFileStorageExt(function (storage) {
        ...
        var refreshView = function () {
            ...
            domFileStorage.readEvent(fileName, function (fileContents) {
                ...
                var strg = new MemoryStorage(ev.storage, ev.model, ev.primaryKey, cache, function () { });
                var eventTemplate = DataTemplate.eventTemplate;
                var currentEvent = new Event(ev, cache, domFileStorage, eventTemplate);

                var categories = $.map(cache.PodiumCategory.id, function (cat) { return cat; });

                var newHtmlContent = [];
                var refreshCallback = !firstLoaded ? onAllGridsAppended : onAllGridsReloaded;
                ...
                var appendCategoryGrid = function (i) {
                    ...
                    if (currentEvent.settings.timingType != "multi-group-start" ||
                        cache.CategoryStart.podiumCategoryId[categories[i].id]) { // check if category started

                        var grid = new ResultsByCategoryGrid(categories[i], currentEvent, timing, appComponents.Toolbar, appComponents.Grid, function () {
        ...
        var appendLogo = function () {
            $("#print_cont").children().first().find(".toolbar").append($("<img>", { src: logoSrc, "class": "print-logo" }));
        }
