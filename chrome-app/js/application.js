var Application = function () {
    ...
    var importCategoriesCSV = function (callback) {
        ...
        var categoryImportKeys = {
            "Name": { dataKey: "name", displayName: "Category Name", required: true, unique: true },
            "Laps": { dataKey: "numberOfLaps", displayName: "Number of Laps" }
        }

        var instructionTextHtml = "Upload instructions:<br/>" +
        ...
        openImagePreviewPopup(exampleImageSrc, imageText, function () {
            importCSV(instructionTextHtml, categoryImportKeys, categoryModelTemplate, existentCategories, validationCallback, insertAction, callback);
        });
    }
    ...
    var importRacersCSV = function (callback) {
        ...
        var racerImportKeys = {
            ...
            "Category": { dataKey: "categoryName", displayName: "Category Name", required: true },
        ...
        openImagePreviewPopup(exampleImageSrc, imageText, function () {
            importCSV(instructionTextHtml, racerImportKeys, racerModelTemplate, existentRacers, validationCallback, insertAction, callback);
        });
        
    }


    var importCSV = function (instructionTextHtml, importKeys, modelTemplate, existentJsonList, validationCallback, insertAction, callback) {
        ...
        var getMissingValueErrorMsg = function (label) {
            var result = label + "";
            return (result[0]) ? result[0].toUpperCase() + result.toLowerCase().substr(1, result.length - 1) + " is missing." : "";
        }
        ...
        var convertToJsonAndSave = function (fileInfo) {
            ...
            $.each(importKeys, function (key, columnSettings) {
                ...
                var matches = $.grep(headers, function (header) { return (header + "").toLowerCase() === (key + "").toLowerCase(); });
                var keyInd = (matches && (matches instanceof Array) && matches.length) ? headers.indexOf(matches[0]) : -1;
            ...
            if (missingHeaders.length) {
                missingHeaders = $.makeArray($(missingHeaders).map(function (i, item) { return '"' + item + '"'; }));
            ...
            var gridHeaderRowData = $.makeArray($(headers).map(function (i, item) {
                var key = (headerKeys[i]) ? headerKeys[i] : headers[i];
                return { value: headers[i], dataKey: key };
            }));

            var grid = (function (headerRowDataArray, headerErrorMsg) { // headerRowDataArray: [{value:"",dataKey:""},]
                var gridSettings = {
                    selectable: false,
                    rowKey: 'id',
                    columns: [
                        { label: fileName, dataKey: csvRowValueKey, "class": "csv-row-value" }
                    ]
                }
                $.each(headerRowDataArray, function (i, headerRowData) {
                    gridSettings.columns.push({ label: headerRowData.value, dataKey: headerRowData.dataKey, "class": "json-value" });
                });
                gridSettings.columns.push({ label: (headerErrorMsg || ""), dataKey: csvRowValidationMsgKey, "class": "csv-row-validation" });
                return new appComponents.Grid(gridSettings);

            })(gridHeaderRowData, missingHeadersErrorStr);

            var addRow = function (csvRowValue, rowDataArray, rowErrorMessage) { // rowDataArray: [{value:"",dataKey:"",errorMsg:""},]
                if (rowErrorMessage || $.grep(rowDataArray, function (rowData) { return !!rowData.errorMsg; }).length > 0) {
                    isTotallyValid = false;
                }
                var row = {};
                var errorMsg = "";
                ...
                row[csvRowValidationMsgKey] = errorMsg + (rowErrorMessage ? " " + rowErrorMessage : "") + missingRequiredFieldsErrorStr;
                grid.addRow(row);
                ...
                $.each(invalidCellIndices, function (i, cellIndex) {
                    ...
                    if (!cell.hasClass('invalid')) { cell.addClass('invalid'); }
                });
            }
            ...
            for (var j = 1; j < lines.length; j++) {
                ...
                if (values && values.length == headers.length) {
                    ...
                    for (var i = 0; i < values.length; i++) {
                        ...
                        if (headerKeys[i]) {
                            var columnSettings = importKeysLCase[(headers[i] + "").toLowerCase()];
                            if (columnSettings.required && !values[i]) {
                                isRowValid = false;
                                gridCellData.errorMsg = getMissingValueErrorMsg(columnSettings.displayName);
                            } else {
                                if (columnSettings.unique) {
                                    if (uniquePropHt[headerKeys[i]][(values[i] + "").toLowerCase()] == null) {
                                        uniquePropHt[headerKeys[i]][(values[i] + "").toLowerCase()] = i;
                                    } else {
                                        isRowValid = false;
                                        gridCellData.errorMsg = getDuplicatedValueErrorMsg(columnSettings.displayName);
                                    }
                                }
                    ...
                    if (errorMsgs) {
                        $.each(gridRowData, function (i, cellData) {
                            if (errorMsgs[cellData.dataKey]) {
                                cellData.errorMsg += (cellData.errorMsg ? " " : "") + errorMsgs[cellData.dataKey];
                                isRowValid = false;
                            }
                        });
                    }
            ...
            popup.onOpen(function () {
                setTimeout(function () {
                    ...
                    if (grid.html.height() > popup.body.height()) {
                        grid.divBody.height(grid.divBody.height() - grid.html.height() + popup.body.height());
                    }
                }, 200);
            });
        ...
        self.lib.confirm(instructionTextHtml,
            function (confirmed) {
                if (confirmed) {
    ...
    var openImagePreviewPopup = function (imageSrc, imageText, callback) {
        var openPopup = function (bodyWidth, bodyHeight) {
            var popup = new appComponents.Popup({ width: bodyWidth, height: bodyHeight });
        ...
        var img = $('<img />', { src: imageSrc, style: "width:auto;height:auto;" })
            .load(function () {
                var scrollW = 18;
                var bordersH = 5;
                var imgW = this.width;
        ...
        var previewCont = $('<div />', { 'class': "image-preview-cont", html: [imgCont, text] });
    }
    ...
    var Routing = function (_app) {
        ...
        var allViews = ["app_entry", "category_start", "laps", "racers", "results", "settings", "start"];
        ...
        var onHashChange = function () {
            ...
            if (h.length && h[0] == "#") { h = h.split("#")[1]; }
            if (!h || allViews.indexOf(h) == -1 ||
        ...
        var loadRoute = function (pageName) { // no needed as public yet
            if (firstLoaded) {
                firstLoaded = false;
        ...
        this.loadEntry = function () {
            isEventLoaded() ? loadRoute(entryPageName) : loadRoute(noEventEntryPageName);
        }
        bindEvents();
    }
    ...
    var t = (function () {
        ...
        _self.messageBox = function (message, callback) {
            var popup = new appComponents.Popup({
                height: 100, width: 300
            });
            popup.body.append("<div class='confirm' style='padding: 15px;'>" + message + "</div>");
